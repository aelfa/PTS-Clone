#!/bin/bash
# basic settings
TARGET_FOLDER="$(cat /var/plexguide/server.hd.path)/downloads/nzb/" # find files in this folders
FIND_SAMPLE_SIZE='100M' # files smaller then this are seen as samples and get deleted
# advanced settings
FIND=$(which find)
FIND_BASE_CONDITION_WANTED='-type f -amin +"$(cat /var/plexguide/cloneclean.nzb)"'
